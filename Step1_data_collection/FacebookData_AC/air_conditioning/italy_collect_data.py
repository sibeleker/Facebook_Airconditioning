...
json_dir = "./jsons/"
directory = "./fb keys/"
df_it = pd.read_excel(directory+"italy_regions.xlsx")
it_states = []
for index, row in df_it.iterrows():
    it_states.append({"key": row['key'], "name": row['name'], "country":"IT"},)

gender_dic =  {"name": "FB audience IT",
            "geo_locations": [{"name": "regions", "values": [region]} for region in it_states], #[{"name": "countries", "values": ["RU"]}],
            "genders": [0, 1,2],
            "ages_ranges": [{"min": 18},
                           ],
...
age_dic =  {"name": "FB audience US",
            "geo_locations": [{"name": "regions", "values": [region]} for region in it_states],#[{"name": "regions", "values": [region]} for region in italy_regions],
            "genders": [0],
            "ages_ranges": [{"min":65, },
                            {"min":18, "max":34},
                            {"min":35, "max":64}
...
education_dic = {"name": "FB audience US",
            "geo_locations": [{"name": "regions", "values": [region]} for region in it_states],
            "genders": [0],
            "ages_ranges": [{"min": 18},
                           ],
...
if __name__ == '__main__':
    ...
    for attribute, dic in attribute_dic.items():
        if attribute in ['age']:#, 'age', 'education']:
            input_file = json_dir+'it_{}.json'.format(attribute)
