'''
Created on Jul 17, 2019

@author: eker
'''
from pysocialwatcher import watcherAPI, constants 
from pysocialwatcher.utils import call_request_fb
import pandas as pd
import time
import logging
import json
import time
json_dir = "./jsons/"
directory = './data/'
#df_countries = pd.read_excel("../src/fb_countries.xlsx", sheet_name='Sheet3')
df_countries = pd.read_excel(directory+'fb keys/fb_countries_june21.xls')
df_countries = df_countries[['country', 'country_code']].dropna()
country_list = []
for index, row in df_countries.iterrows():
    if row['country'] not in ['Mozambique', 'Namibia', 'Russia']:
        country_list.append({"name": "countries", "values": [row['country_code']]})

testdic = {"name": "FB audience",
            "geo_locations": country_list, #[{"name": "countries", "values": ['ID']}],
            "genders": [0],
            "ages_ranges": [{"min":15,},
                           ],
           "interests": [{"or": [], "name": "no_interest"},
                         ],
            "publisher_platforms" : ['facebook', 'instagram']
            }

age_dic =  {"name": "FB audience IN",
            "geo_locations": country_list,
            "genders": [0],
            "ages_ranges": [
                            {"min":65, },
                            {"min":18, "max":34},
                            {"min":35, "max":64}
                           ],
            "interests": [{"or": [6003711009918], "name": "Air conditioning"}, 
                         {"or": [], "name": "no_interest"},
                         ],
            "publisher_platforms" : ['facebook', 'instagram']
            }
gender_dic =  {"name": "FB audience IN",
            "geo_locations": country_list, #[{"name": "countries", "values": ["RU"]}],
            "genders": [0, 1,2],
            "ages_ranges": [{"min": 18},
                           ],
            "interests": [{"or": [6003711009918], "name": "Air conditioning"}, 
                         {"or": [], "name": "no_interest"},
                         ],
            "publisher_platforms" : ['facebook', 'instagram']
            }

education_dic = {"name": "FB audience IN",
            "geo_locations": country_list,
            "genders": [0],
            "ages_ranges": [{"min": 18},
                           ],
            "scholarities":[  {"name" : "1 High school student", "or" : [1]},
                          {"name" : "2 Some high school", "or" : [13]},
                          {"name" : "3 High school grad or studying or some college", "or" : [2, 4, 5]},
                          {"name" : "4 Professional or associate degree", "or" : [6, 10]},
                          {"name" : "5 College grad or studying higher", "or" : [3, 7, 8]},
                          {"name" : "6 Masters and Doctorate degree", "or" : [9, 11]},
                          {"name" : "7 Unspecified", "or" : [12]}
                       ],
            "interests": [{"or": [6003711009918], "name": "Air conditioning"}, 
                         {"or": [], "name": "no_interest"},
                         ],
            "publisher_platforms" : ['facebook', 'instagram']
            }

relationship_dic = {"name": "FB audience",
            "geo_locations": country_list,
            "genders": [0],
            "ages_ranges": [{"min": 18},
                           ], 
            "relationship_statuses": [{"name" : "married or living together", "or" : [3, 7, 8]},
                                      {"name" : "all ", "or" : []}] ,
                                     #{"name" : "all others relationship", "or" : [1, 2, 4, 6, 9, 10, 11, 12, 13]}] ,
            "interests": [{"or": [6003711009918], "name": "Air conditioning"}, 
                         {"or": [], "name": "no_interest"},
                         ],
            "publisher_platforms" : ['facebook', 'instagram']
            }
family_dic = {"name": "FB audience",
            "geo_locations": country_list,
            "genders": [0],
            "ages_ranges": [{"min": 18},
                           ], 
            "family_statuses" : [{"name" : "parents of 0-8 yrs old", "or" : [6023005372383, 6023005458383, 6023005529383, 6023005570783]},
                             {"name" : "parents of 9-18 yrs old", "or" : [6023080302983, 6023005681983]},
                             {"name" : "all family", "or" : []}],
            "interests": [{"or": [6003711009918], "name": "Air conditioning"}, 
                         {"or": [], "name": "no_interest"},
                         ],
            "publisher_platforms" : ['facebook', 'instagram']
            }

attribute_dic = {'gender' : gender_dic,
                 'age' : age_dic, 
                 'education' : education_dic, 
                 
                 'family' : family_dic, 
                 'relationship' : relationship_dic}


if __name__ == '__main__':
    logging.getLogger("urllib3").setLevel(logging.WARNING)
    logging.getLogger("requests").setLevel(logging.WARNING)
    

    watcher = watcherAPI()   
    token = # ENTER YOUR TOKEN !!!
    account = # PLEASE ENTER YOUR ACCOUNT ID !!!

    watcher.add_token_and_account_number(token, account)  
    watcher.check_tokens_account_valid()
    '''
    input_file = json_dir+'all_countries_test.json'
    with open(input_file, 'w') as fp:
        json.dump(testdic, fp)
    watcher.run_data_collection(input_file)
    '''


    for attribute, dic in attribute_dic.items():
        if attribute in ['family']:
            print(attribute)
        #if attribute != 'age':
            input_file = json_dir+'all_countries_{}.json'.format(attribute)
            with open(input_file, 'w') as fp:
                json.dump(dic, fp)
            watcher.run_data_collection(input_file)
            print("Attribute {} is done.".format(attribute))
